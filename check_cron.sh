...
VMESS_PATH="/home/${USER}/.vmess"

CRON_S5="nohup ${FILE_PATH}/s5 -c ${FILE_PATH}/config.json >/dev/null 2>&1 &"
CRON_NEZHA="nohup ${WORKDIR}/start.sh >/dev/null 2>&1 &"
CRON_VM="nohup ${VMESS_PATH}/bot tunnel --edge-ip-version auto --config tunnel.yml run >/dev/null 2>&1 & # vmess"
...
if [ "$(command -v pm2)" == "/home/${USER}/.npm-global/bin/pm2" ]; then
  ...
  (crontab -l | grep -F "# vmess") || (crontab -l; echo "*/12 * * * * pgrep -x \"bot\" > /dev/null || ${CRON_VM}") | crontab -  # 添加 vmess 保活的 crontab
else
